function net = rbf(nin, nhidden, nout, rbfunc, outfunc, prior)
%RBF	Creates an RBF network with specified architecture
%
%	Description
%	NET = RBF(NIN, NHIDDEN, NOUT, RBFUNC, OUTFUNC, PRIOR) constructs and initialises a
...
%       'logistic' for logistic outputs and CE error.
%       'softmax' for softmax outputs and CE error.
...
%	specify the type of error function to be used.  The field OUTFUNC is
...
%	supported.
%
%	NET = RBF(NIN, NHIDDEN, NOUT, RBFUNC, OUTFUNC, PRIOR), in which
...
%	with inverse variance with value PRIOR. 
%
%
%	See also
...
outfns = {'linear', 'logistic', 'softmax', 'neuroscale'};
