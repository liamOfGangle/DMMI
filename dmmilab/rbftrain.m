...
switch net.outfn
    case 'linear'
        ...
        elseif size(net.alpha == [1 1])
            ...
            hessian = Phi'*Phi + net.alpha*eye(net.nhidden+1); % Corrected by RFH 1 May 2003
        ...
        net.w2 = temp(1:net.nhidden, :);
        net.b2 = temp(net.nhidden+1, :);
        options(8) = rbferr(net, x, t); % introduced by RFH to emulate glmtrain
        return % end linear case
    ...
    case 'neuroscale'
        ...
        return % end neuroscale case
    case 'logistic'
        Phi = [act, ones(ndata, 1)];
        e = ones(1, net.nhidden+1);
        for n = 1:options(14)
            if n == 1
                % Initialise model
                p = (t+0.5)/2;
            ...
            if (min(min(weights)) < eps)
                warning('ill-conditioned weights in rbftrain')
                return
            end
            z = act + (t-p)./link_deriv;
            % Treat each output independently with relevant set of weights
            for j = 1:net.nout
                indep = Phi.*(weights(:,j)*e);
                dep = z(:,j).*weights(:,j);
                temp = indep\dep;
                net.w2(:,j) = temp(1:net.nhidden);
                net.b2(j) = temp(net.nhidden+1);
            end
            [err, edata, eprior, p, act] = rbferr(net, x, t);
            if n == 1
                errold = err;
                wold = netpak(net);
            else
    ...
    case 'softmax'
        ...
        for n = 1:options(14)
            ...
            if options(5) == 1 | n == 1
                ...
                w = wold + deltaw;
                net.w2(:,j) = w(1:net.nhidden);
                net.b2(j) = w(net.nhidden+1);
                [err, edata, eprior, p] = rbferr(net, x, t);
            end
            if options(1)
                fprintf(1, 'Cycle %4d Error %11.6f\n', n, err)
            end
